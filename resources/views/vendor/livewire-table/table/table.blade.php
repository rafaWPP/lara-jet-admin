<table class="w-full relative" x-data="{ selected: @entangle('selected') }">
    ...
    <tr class="group">
        @if($this->canSelect())
            <th class="p-0 text-left text-black bg-neutral-50 dark:text-gray-500 dark:bg-neutral-800">
    ...
    <tr class="group">
        @if($this->canSelect())
            <th class="p-0 text-left text-black bg-neutral-50 dark:text-gray-500 dark:bg-neutral-800"></th>
        @endif
        @foreach($table['columns'] as $column)
            @continue(! in_array($column->code(), $this->columns))
            <th class="p-0 text-left text-black bg-neutral-50 dark:text-gray-500 dark:bg-neutral-800">
    ...
    @if($this->deferLoading && ! $this->initialized)
        <tr class="group">
            <td class="p-0" colspan="{{ $table['columns']->count() + 1 }}">
                <span class="block text-lg text-center py-20 bg-white text-black dark:bg-neutral-900 dark:text-gray-500">
